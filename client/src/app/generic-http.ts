...
export class GenericHttpService<T> {
  ...
  getById(id: number): Observable<T> {
    ...
    return this.http.get<T>(urlWithId).pipe(retry(1), catchError(this.handleError));
  }
  ...
  update(data: T): Observable<T> {
    ...
    return this.http.put<T>(urlWithId, data).pipe(retry(1), catchError(this.handleError));
  }
  delete(id: number): Observable<number> {
    const urlWithId = `${BASE_URL}/${this.endPoint}/${id}`;
    return this.http.delete<number>(urlWithId).pipe(retry(1), catchError(this.handleError));
  }
