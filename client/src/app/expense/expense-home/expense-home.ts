...
import { EXPENSE_DEFAULT } from '../../constants';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatComponentsModule, EmployeeModule, ExpenseDetail],
...
export class ExpenseHome implements OnInit {
  ...
  getAllExpenses(verbose: boolean = true): void {
    this.expenseService.getAll().subscribe({
      ...
      error: (e: Error) => (this.msg = `Failed to load expenses - ${e.message}`),
  ...
  create(expense: Expense): void {
    ...
    this.expenseService.create(expense).subscribe({
      next: (e: Expense) => {
        this.msg = e.id > 0 ? `Expense ${e.id} added!` : `Expense ${e.id} not added!`;
  ...
  sortExpensesWithObjectLiterals(sort: Sort): void {
    const literals = {
      id: () =>
        (this.dataSource.data = this.dataSource.data.sort((a: Expense, b: Expense) =>
